...
void testEngineeringValueEncoding()
{
    TEST_ASSERT_EQUAL_INT16(
        123, BattlebotTelemetry::encodeAccelerationCentiG(1.234f));
    TEST_ASSERT_EQUAL_INT16(
        -10000, BattlebotTelemetry::encodeAccelerationCentiG(-120.0f));
    TEST_ASSERT_EQUAL_INT16(
        0, BattlebotTelemetry::encodeAccelerationCentiG(NAN));
    TEST_ASSERT_EQUAL_UINT16(
        17321, BattlebotTelemetry::encodePeakMagnitudeCentiG(200.0f));
    TEST_ASSERT_EQUAL_INT16(
        -1234, BattlebotTelemetry::encodeRpm(-1234.4f));
    TEST_ASSERT_EQUAL_INT16(
        32767, BattlebotTelemetry::encodeRpm(50000.0f));
}
...
void testFrameLayoutAndCrc()
{
    ...
    const size_t size = BattlebotTelemetry::buildFrame(
        frame, sizeof(frame), 0x5A, snapshot);

    const uint8_t expected[] = {
        0xC8, 0x18, 0x80, 0xF3, 0x01, 0x5A, 0x7F,
        0xFB, 0x2E,
        0x00, 0x7B, 0xFE, 0x38, 0x03, 0x15,
        0xD8, 0xF0, 0x00, 0x00, 0x27, 0x10,
        0x27, 0x11, 0x43, 0xA9,
        0x7C,
    };
...
void testSmallOrNullOutputIsRejected()
{
    ...
    TEST_ASSERT_EQUAL_UINT32(
        0,
        BattlebotTelemetry::buildFrame(
            shortFrame, sizeof(shortFrame), 0, snapshot));
    TEST_ASSERT_EQUAL_UINT32(
        0,
        BattlebotTelemetry::buildFrame(
            nullptr, BattlebotTelemetry::FRAME_SIZE, 0, snapshot));
}
