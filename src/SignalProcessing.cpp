...
int16_t mapRange(uint16_t value, uint16_t inputMin, uint16_t inputMax, int16_t outputMin, int16_t outputMax) {
    const int32_t inputPosition = static_cast<int32_t>(value) - inputMin;
    const int32_t inputRange = static_cast<int32_t>(inputMax) - inputMin;
    const int32_t outputRange = static_cast<int32_t>(outputMax) - outputMin;

    return static_cast<int16_t>(outputMin + (inputPosition * outputRange) / inputRange);
}

int16_t processStick(uint16_t channelUs) {
    const uint16_t value = clampChannel(channelUs);
    const uint16_t deadbandLow = RobotConfig::RC_CHANNEL_CENTER - RobotConfig::RC_DEADBAND;
    const uint16_t deadbandHigh = RobotConfig::RC_CHANNEL_CENTER + RobotConfig::RC_DEADBAND;

    if(value < deadbandLow) {
        return mapRange(value, RobotConfig::RC_CHANNEL_MIN, deadbandLow, -RobotConfig::RC_OUTPUT_SCALE, 0);
    }

    if(value > deadbandHigh) {
        return mapRange(value, deadbandHigh, RobotConfig::RC_CHANNEL_MAX, 0, RobotConfig::RC_OUTPUT_SCALE);
    }
...
int16_t processPotentiometer(uint16_t channelUs) {
    ...
    if(value <= RobotConfig::RC_CHANNEL_CENTER) {
        return mapRange(value, RobotConfig::RC_CHANNEL_MIN, RobotConfig::RC_CHANNEL_CENTER,
                        -RobotConfig::RC_OUTPUT_SCALE, 0);
    }

    return mapRange(value, RobotConfig::RC_CHANNEL_CENTER, RobotConfig::RC_CHANNEL_MAX, 0,
...
#define RECEIVER_PROCESS_FIELD(name, type, channel) input.name = process##type(channels.channelsUs[(channel) - 1]);
